...
import PostPreviewA from '../components/profile components/PostPreviewA.jsx';
...
function Profile() {
    ...
    useEffect(() => {
        ...
        if (isPopupVisible) {
            gsap.from(popupBoxCover, {
                opacity: 0,
                duration: 0.1
            });
        } else {
            closePopupBtn?.addEventListener('click', () => {
                popupBoxCover.style.opacity = 100;
                popupBoxCover.style.animationDuration = '0.3s';
            })
        }
    }, [isPopupVisible]);
    ...
    // Handle profile edit tab open and close
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        return () => {
            previewCover?.removeEventListener('click', closePreview);
        };
    }, []);

    return (
        <div className={`main-container w-full h-full ${isPopupVisible ? 'fixed' : 'static'}`}>
            <LoadingBar />
            ...
            <main className=''>
                <div className="profile-main-container">
                    ...
                    <div onClick={() => openPopup(popupInner[0])} className='popup-open-btn popup-open-btn1 w-full cursor-pointer select-none' title='About'>
                            <PopupBtn props={{ name: 'About', visible: true }} />
                        </div>
                        <div onClick={() => openPopup(popupInner[1])} className={linkVisible === true ? `popup-open-btn popup-open-btn2 w-full cursor-pointer select-none` : 'hidden'} title={linkTitle}>
                            <PopupBtn props={{ name: linkTitle, visible: linkVisible }} />
                        </div>
                        {isPopupVisible && (
                            <div id='popup-box-cover' className="popup-box-cover w-full h-screen p-4 fixed top-0 left-0 z-[99] flex items-center justify-center bg-[#00000098] backdrop-blur-sm md:p-8">
                                <div className="popup-box p-4 bg-white rounded-3xl min-h-64 max-h-96 w-full relative lg:w-[524px]">
                                    <div className="close-popup absolute right-3 top-3 p-1 active:bg-zinc-300 rounded-full duration-200 cursor-pointer" onClick={closePopup}>
